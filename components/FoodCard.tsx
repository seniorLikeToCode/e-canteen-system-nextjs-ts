...
export default function FoodCard({ category, products }: { category: string; products: IProducts }) {
    ...
    const addToCart = async (id: string) => {
        // console.log(`https://ecs-api-dev.onrender.com/api/v1/cart/add/${id}`);
        const addItem = {
            method: 'get',
                                    ...
                                    <div key={product._id} className='rounded-lg p-2 shadow-md bg-slate-50'>
                                        ...
                                        <div className='flex flex-col items-center mt-3'>
                                            <div className=' w-32 h-32 rounded-full overflow-hidden bg-slate-200'>
                                                <Image className='mt-3 scale-150' loader={() => product.imageUrl} unoptimized={true} width={120} height={120} src={product.imageUrl} alt="food items" />
                                            </div>
                                        </div>
